...
class Atende(models.Model):
    # The composite primary key (medico, convenio) found, that is not supported. The first column is selected.
    medico = models.OneToOneField('Medico', models.DO_NOTHING, db_column='medico', primary_key=True)
    ...
    def __str__(self):
        return f'Doutor {self.medico.nome} atende convênio {self.convenio.nome}'
...
class Consulta(models.Model):
    ...
    def __str__(self):
        return f'Consulta {self.paciente.nome} com o Dr. {self.medico.nome}'
...
class Possui(models.Model):
    # The composite primary key (paciente, convenio) found, that is not supported. The first column is selected.
